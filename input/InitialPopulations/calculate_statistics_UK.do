...
import delimited "C:\Users\Patryk\git\labsim\input\InitialPopulations\population_initial_UK_`year'.csv", clear
...
*Monetary values in the estimation sample are in 2015 values. Revert to nominal value for each year.
*Income CPI from ONS:
gen CPI = .
replace CPI = 0.866 if stm == 2009
replace CPI = 0.894 if stm == 2010
replace CPI = 0.934 if stm == 2011
replace CPI = 0.961 if stm == 2012
replace CPI = 0.985 if stm == 2013
replace CPI = 1.000 if stm == 2014
replace CPI = 1.000 if stm == 2015
replace CPI = 1.007 if stm == 2016
replace CPI = 1.034 if stm == 2017
replace CPI = 1.059 if stm == 2018
replace CPI = 1.078 if stm == 2019

gen grossearnings = sinh(yplgrs_dv)*12*CPI // Multiply by the CPI to revert previous CPI-adjustment
drop if grossearnings <= 0 | grossearnings >= 5000000
...
gen hourlywage = sinh(yplgrs_dv)*CPI/(lhw*4.33) // Multiply by the CPI to revert previous CPI-adjustment
drop if hourlywage <= 0 | hourlywage >= 1000
