...
public class SQLdataParser {
	...
	public static void createDatabaseTablesFromCSVfile(Country country, String inputFilename, String initialInputFilename, int startYear, Connection conn) {
    ...
    //    	SQLdataParser.parse(Parameters.INPUT_DIRECTORY + initialInputFilename + ".csv", initialInputFilename, false, conn, country, startYear, true);
			SQLdataParser.parse(Parameters.INPUT_DIRECTORY_INITIAL_POPULATIONS + initialInputFilename + ".csv", initialInputFilename, false, conn, country, startYear, true);
...
//	        SQLdataParser.parse(Parameters.INPUT_DIRECTORY + inputFilename + ".csv", inputFilename, false, conn, country, startYear, false);
			...
			SQLdataParser.parse(Parameters.INPUT_DIRECTORY + inputFilename + ".csv", inputFilename, true, conn, country, startYear, false);
	}
	...
	private static void parse(String inputFileLocation, String inputFileName, boolean donorTables, Connection conn, Country country, int startyear, boolean initial) {
		
		//If initial set to true, do the following:
		if (initial) {
			
			//Set name of tables
			String personTable = "person_" + country;
			String benefitUnitTable = "benefitUnit_" + country;
			String householdTable = "household_" + country;
...
//	    	        		+ "UPDATE " + benefitUnitTable + " SET idfemale = null WHERE idfemale = 0;"
		...
		String personTable = "person_" + country;
		String householdTable = "household_" + country;
		if(donorTables) {		//Used to rename tables at the end to store in input database
			personTable = "DonorPerson_" + country;
			householdTable = "DonorHousehold_" + country;
		}
...
//	        		+ "ALTER TABLE " + personTable + " ALTER COLUMN dag RENAME TO age;"
	        		...
	        		//Labour Market Economic Status
					//Retired now put in NotEmployed to match new categories (TODO: make sure this is required, or donor population should be kept unchanged)
	        		+ "ALTER TABLE " + personTable + " ADD activity_status VARCHAR_IGNORECASE;"
...
//	        		+ "UPDATE " + personTable + " SET activity_status = 'Retired' WHERE les = 4;"
	        		+ "UPDATE " + personTable + " SET activity_status = 'NotEmployed' WHERE les = 5 OR les = 4 OR les >= 7;"
