...
//		yearlySchedule.addEvent(this, Processes.Timer);
		...
		//2A - HEALTH MODULE
		// 2A: Update Health - determine health (continuous) based on regression models: done here because health depends on education
		yearlySchedule.addCollectionEvent(persons, Person.Processes.Health);

		
		//3 - HOUSEHOLD COMPOSITION MODULE: Decide whether to enter into a union (marry / cohabitate), and then perform union matching (marriage) between a male and female

		// A: Update potential earnings so that as up to date as possible to decide partner in union matching.  Note that the responsible male and/or female of each household also has their potential earnings adjusted in the labour market module
		// TODO: Check whether it is appropriate to update here AND labour market module.  The labour market module is necessary because the potential earnings are adjusted as part of the labour demand-supply convergence process, so it 
		//is necessary to calculate there.  Given the iterative nature of the labour market module, it is more efficient to only update the potential earnings of those responsible adults (not retired, students or children) in the labour demand, 
		// rather than everyone.  Hence, we need to update everyone here, before the union matching module (and after the previous time-steps' labour market module).
		//PB: so if not using labour demand, possibly potential earnings only have to be updated once, here? 
...
//			log.debug("Person " + person.getKey().getId() + ", gender " + gender + ", region " + region + ", age " + age + " has Weight = " + person.getWeight());
	...
	private void populationAlignmentUnweighted() {
		...
		*/
		...
		//TODO: Code creating new persons by gender, region, age here
		for (Gender gender : Gender.values()) {
			for (Region region : Parameters.getCountryRegions()) {
    	...
    	//For each of the cloned individuals decide if they should go to cohabitation, personsToMatch, or are children
    			...
    			for(Person person : clonedPersonsPopulationAlignment) {
    				if(person.getDag() >= Parameters.MIN_AGE_COHABITATION) { //If above age to create HH, consider which process should be followed
    					...
    					if(person.getOriginalHHStatus().equals(Household_status.Couple)) {
    						//TODO: what happens to people who were in a couple
    						personsToMatch.get(person.getDgn()).get(person.getBenefitUnit().getRegion()).add(person); //If cloned person was in a couple, skip considerCohabitation and add directly to matching pool
    						...
    						//TODO: Singles will follow standard process so nothing to do here? They will go to considerCohabitation according to the yearly schedule
    						tmpClonedPersonsRemaining --;
    					}
    				} 
...
//			throw new IllegalArgumentException("Number of " + gender.toString() + "s of Age " + age + " in simulation is not positive!  Impossible to clone to create more persons of same Age and Gender!");
		...
		if (numPersonsInSimMinusTarget > 0) {
...
//				System.out.println("There are " + unmatchedMales.size() + " unmatched males and " + unmatchedFemales.size() + " unmatched females at the end. Number of matches made for " + region + " is " + matches.size());
...
//		System.out.println("There are " + unmatchedMales.size() + " unmatched males and " + unmatchedFemales.size() + " unmatched females at the end. Number of matches made " + matches.size() + " and total number of matches in all years is " + allMatches);
	...
	private void considerCohabitationAlignment() {
		...
		for (Person person : persons) {
			if (person.getDag() >= 29 && person.getPartner() == null) {
		...
		int targetNumberToBePartnered = (int) ( (double) numPersonsWhoCanBePartnered * 0.20); // - numPersonsToBePartnered;
	...
	private void employmentAlignment() {
		...
		final EnumSet<Region> regionsOfItaly = EnumSet.of(Region.ITC, Region.ITH, Region.ITI, Region.ITF, Region.ITG);

		for (Gender gender : Gender.values()) {
			personsByGenderAndRegion.put(gender, new LinkedHashMap<Region, Set<Person>>());
			for (Region region : regionsOfItaly) {
...
//        }
	 ...
	 * 
	 * @param euromodOutputPersons
	 */
	private void createInitialPopulationDataStructures(Map<Long, DonorPerson> euromodOutputPersons) {
