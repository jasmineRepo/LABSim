...
//	private MultiKeyMap disposableIncomeUprated;
	...
	@Transient
	private Map<String, Double> disposableIncomeNormalised; // Stores policy-specific value of disposable income which has been normalised by the median disposable income of donor households for a given policy
...
//	private double grossEarnings; //When earnings = 0 it is not possible to calculate the income to gross earnings ratio
	...
	public DonorHousehold() {
		...
		otherMembers = new LinkedHashSet<>();
		disposableToGrossIncomeRatio = new LinkedHashMap<>();
		disposableIncomeToGrossEarningsRatio = new LinkedHashMap<>();
		disposableIncome = new LinkedHashMap<>();
		grossEarnings = new LinkedHashMap<>();
		grossIncome = new LinkedHashMap<>();
		disposableIncomeNormalised = new LinkedHashMap<>();
	//	disposableIncomeUprated = new MultiKeyMap<>();
...
//	}
	...
	public double getGrossIncome(String euromodPolicyName) {
		return grossIncome.get(euromodPolicyName);
	}
	...
	*/
	...
	// Get disposable income of the donor benefit unit, normalised by the gross income median for a given policy
	public double getDisposableIncomeNormalised() {
		return disposableIncomeNormalised.get(model.getLabourMarket().getEUROMODpolicyNameForThisYear());
	}

	public void setDisposableIncomeNormalisedForPolicy(String euromodPolicyYear, double disposableIncomeNormalised) {
		this.disposableIncomeNormalised.put(euromodPolicyYear, disposableIncomeNormalised);
	}
