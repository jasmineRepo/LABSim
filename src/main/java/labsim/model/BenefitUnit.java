...
//		this.household = originalBenefitUnit.household;
	...
	protected void updateResponsiblePersons() {
...
//			}
	...
	public double getDoubleValue(Enum<?> variableID) {
		...
		case IncomeDiv100: 							//Disposable monthly income from donor household divided by 100
			return disposableIncomeMonthly * 1.e-2;
		case IncomeSqDiv10000:						//Income squared divided by 10000
			return disposableIncomeMonthly * disposableIncomeMonthly * 1.e-4;
		case IncomeDiv100_MeanPartnersAgeDiv100:		//Income divided by 100 interacted with mean age of male and female in the household divided by 100
			if(female == null) {		//Single so no need for mean age
				return disposableIncomeMonthly * male.getDag() * 1.e-4;
			}
			else if(male == null) {
				return disposableIncomeMonthly * female.getDag() * 1.e-4;
			}
			else {		//Must be a couple, so use mean age
				double meanAge = (female.getDag() + male.getDag()) * 0.5;
				return disposableIncomeMonthly * meanAge * 1.e-4;
			}
		case IncomeDiv100_MeanPartnersAgeSqDiv10000: 	//Income divided by 100 interacted with square of mean age of male and female in the household divided by 10000
			if(female == null) {		//Single so no need for mean age
				return disposableIncomeMonthly * male.getDag() * male.getDag() * 1.e-6;
			}
			else if(male == null) {
				return disposableIncomeMonthly * female.getDag() * female.getDag() * 1.e-6;
			}
			else {		//Must be a couple, so use mean age
				double meanAge = (female.getDag() + male.getDag()) * 0.5;
				return disposableIncomeMonthly * meanAge * meanAge * 1.e-6;
			}
		...
		case MaleLeisure_IncomeDiv100:
			return (Parameters.HOURS_IN_WEEK - male.getLabourSupplyWeekly().getHours()) * disposableIncomeMonthly * 1.e-2;
		...
		case FemaleLeisure_IncomeDiv100:
			return (Parameters.HOURS_IN_WEEK - female.getLabourSupplyWeekly().getHours()) * disposableIncomeMonthly * 1.e-2;
		...
		case HoursMaleByIncome:
			return male.getLabourSupplyWeekly().getHours() * disposableIncomeMonthly * 1.e-3;
		case HoursFemaleByIncome:
			return female.getLabourSupplyWeekly().getHours() * disposableIncomeMonthly * 1.e-3;
		...
		case IncomeByAge:		//Use mean age for couples
			if(female == null) {		//Single so no need for mean age
				return disposableIncomeMonthly * male.getDag() * 1.e-1;
			}
			else if(male == null) {
				return disposableIncomeMonthly * female.getDag() * 1.e-1;
			}
			else {		//Must be a couple, so use mean age
				double meanAge = (female.getDag() + male.getDag()) * 0.5;
				return disposableIncomeMonthly * meanAge * 1.e-1;
			}
		case IncomeByAgeSquared:		//Use mean age for couples
			if(female == null) {		//Single so no need for mean age
				return disposableIncomeMonthly * male.getDag() * male.getDag() * 1.e-2;
			}
			else if(male == null) {
				return disposableIncomeMonthly * female.getDag() * female.getDag() * 1.e-2;
			}
			else {		//Must be a couple, so use mean age
				double meanAge = (female.getDag() + male.getDag()) * 0.5;
				return disposableIncomeMonthly * meanAge * meanAge * 1.e-2;
			}
		case IncomeByNumberChildren:
			return disposableIncomeMonthly * children.size();
		case HoursMale:
