...
//	private double hourlyWageGross;		//To be compared with the simulated Person objects' 'potentialEarnings' field
	...
	@Transient
	private Map<String, Boolean> receivesBenefitsFlag;
...
//		key = new PanelEntityKey(idPerson);
		...
		receivesBenefitsFlag = new LinkedHashMap<>();
...
//    		selfEmploySocialInsurancePerHour.put(policyName, model.getDonorPersonEUROMODpolicyDependentVariablesByIdPolicyNameAndAttribute().get(key.getId(), policyName, Parameters.SELF_EMPLOY_SOCIAL_INSURANCE_VARIABLE_NAME));
			...
			receivesBenefitsFlag.put(policyName, (model.getDonorPersonEUROMODpolicyDependentVariablesByIdPolicyNameAndAttribute().get(key.getId(), policyName, Parameters.ILS_BENMT_NAME) > 0 || model.getDonorPersonEUROMODpolicyDependentVariablesByIdPolicyNameAndAttribute().get(key.getId(), policyName, Parameters.ILS_BENNT_NAME) > 0));
        }
	}
...
//	        		.append(selfEmploySocialInsurancePerHour)
	...
	public boolean getReceivesBenefitsFlag() {
		 return receivesBenefitsFlag.get(model.getLabourMarket().getEUROMODpolicyNameForThisYear());
	}
