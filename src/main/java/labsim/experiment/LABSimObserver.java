...
import labsim.data.filters.*;
import labsim.model.BenefitUnit;
import labsim.model.Validator;
...
import labsim.model.enums.HistogramTypeEnum;
 ...
 *
 */
public class LABSimObserver extends AbstractSimulationObserverManager implements EventListener, ILongSource {
...
//	private Weighted_CrossSection.Double wagesAtRiskOfWorkCS;
	 ...
	 *
	 */
	public LABSimObserver(SimulationManager manager, SimulationCollectorManager simulationCollectionManager) {
	...
	public void buildObjects() {
		...
		if(showCharts) {
			...
			final LABSimCollector collector = (LABSimCollector) getCollectorManager();
...
//			histogramPlotSet = new LinkedHashSet<Weighted_HistogramSimulationPlotter>();
			...
			for(Region region: Parameters.getCountryRegions()) {
				for(Education edu: Education.values()) {
					...
					wagesCS = new Weighted_CrossSection.Double(model.getPersons(), Person.class, "getPotentialEarnings", true);
...
//					wagesAtRiskOfWorkCS = new Weighted_CrossSection.Double(model.getPersons(), Person.class, "getPotentialEarnings", true);
...
//					populationAgePlotter.addSeries("80 - 100 official projection", model, "getPopulationProjectionByAge80_100", true);
		...
		//		activityStatusPlotter.addSeries("Not Employed", new Weighted_MeanArrayFunction(notEmployedCS), null, colorArrayList.get(1), false);
		...
		//		activityStatusPlotter.addSeries("Retired validation", validator, Validator.DoublesVariables.activityStatus_Retired, colorArrayList.get(3), true);
			...
			if(studentsByRegion && showAdditionalCharts) {
			    ...
			    if (showValidationStatistics) {
			    	studentRegionPlotter.addSeries("Validation Nord Ovest", validator, Validator.DoublesVariables.studentsByRegion_ITC, colorArrayList.get(0), true);
					studentRegionPlotter.addSeries("Validation Sud", validator, Validator.DoublesVariables.studentsByRegion_ITF, colorArrayList.get(1), true);
					studentRegionPlotter.addSeries("Validation Isole", validator, Validator.DoublesVariables.studentsByRegion_ITG, colorArrayList.get(2), true);
					studentRegionPlotter.addSeries("Validation Nord Est", validator, Validator.DoublesVariables.studentsByRegion_ITH, colorArrayList.get(3), true);
					studentRegionPlotter.addSeries("Validation Centro", validator, Validator.DoublesVariables.studentsByRegion_ITI, colorArrayList.get(4), true);
				}
...
//			    tabSet.add(createScrollPaneFromPlots(healthAgePlots, "Health: age/gender", 2));			
		...
		//			int ageTo = ageFilter.getAgeTo();
					...
					MaleAgeGroupCSfilter maleAgeFilterDisabled = new MaleAgeGroupCSfilter(0, 100);
					FemaleAgeGroupCSfilter femaleAgeFilterDisabled = new FemaleAgeGroupCSfilter(0, 100);
...
//					FemaleRegionCSfilter femaleRegionFilter = new FemaleRegionCSfilter(region);
		    ...
		    if(labourSupply) {
				...
				int colorCounter = 0;
				for(Education edu: Education.values()) {
					EducationCSfilter eduFilter = new EducationCSfilter(edu);
					...
					supplyPlotter.addSeries(edu.toString(), new Weighted_SumArrayFunction.Double(supplyCS), null, colorArrayList.get(colorCounter), false);
//					supplyPlotter.addSeries("Validation " + edu.toString(), validator, Validator.DoublesVariables.valueOf("labour_supply_"+edu), colorArrayList.get(colorCounter), true);
		    ...
		    if(grossEarningsByRegionAndEducation) {
		    	IndividualBarSimulationPlotter earningsPlotter = new IndividualBarSimulationPlotter("Yearly Gross Earnings by Education and Region (excludes non-workers)", "Euro");
				for(Region region: Parameters.getCountryRegions()) {
		    		for(Education edu: Education.values()) {
...
//			getEngine().getEventQueue().scheduleOnce(chartingEvents, model.getStartYear(), Order.BEFORE_ALL.getOrdering()+1);
	...
	private Color colorOfEducation(Education edu) {
		...
		else throw new IllegalArgumentException("ERROR - no color is specified for " + edu + " in LABSimObserver class!");
	}
...
//		System.out.println("Potential Earnings of person " + p.getKey().getId() + ": count " + countIterations + ", " + p.getRegion() + ": " + p.getEducation() + ", " + p.getPotentialEarnings());
...
//			resetConvergencePlotter();		    
	...
	public long getLongValue(Enum<?> var) {
		...
		default:
			throw new IllegalArgumentException("ERROR - " + var + " not found in LABSimObserver#getLongValue()");
