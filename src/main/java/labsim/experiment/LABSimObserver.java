...
//			    tabSet.add(createScrollPaneFromPlots(healthAgePlots, "Health: age/gender", 2));			
		...
		//		}
				...
				for (AgeGroupCSfilter ageFilter : healthMentalAgeGroupFilterSet) {
					...
					TimeSeriesSimulationPlotter healthMentalAgePlotter = new TimeSeriesSimulationPlotter("Mental health score by age: " + ageFilter.getAgeFrom() + " - " + ageFilter.getAgeTo(), "");
				...
				tabSet.add(createScrollPaneFromPlots(healthMentalAgePlots, "Mental health: age/gender", 2));
...
//					FemaleRegionCSfilter femaleRegionFilter = new FemaleRegionCSfilter(region);
			...
			if (grossEarningsByRegionAndEducation) {
				...
				for(Education edu: Education.values()) {
					for (Gender gender : Gender.values()) {
						GenderEducationWorkingCSfilter genderEducationWorkingFilter = new GenderEducationWorkingCSfilter(gender, edu);
						Weighted_CrossSection.Double wagesCS = new Weighted_CrossSection.Double(model.getPersons(), Person.class, "getPotentialEarnings", true); // Note: these are nominal values for each simulated year
						wagesCS.setFilter(genderEducationWorkingFilter);
			...
			if (grossEarningsByRegionAndEducation) {
				...
				for (Gender gender : Gender.values()) {
					...
					Weighted_CrossSection.Double hoursCS = new Weighted_CrossSection.Double(model.getPersons(), Person.class, "getLabourSupplyWeeklyHours", true); // Note: these are nominal values for each simulated year
